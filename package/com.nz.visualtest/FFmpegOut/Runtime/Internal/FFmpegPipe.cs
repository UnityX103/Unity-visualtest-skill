...
namespace FFmpegOut
{
    public sealed class FFmpegPipe : System.IDisposable
    {
        ...
        {
            get {
                var bundledPath = GetBundledExecutablePath();
                if (File.Exists(bundledPath))
                    return bundledPath;

                return ResolvePathExecutable();
            }
        }

        static string GetBundledExecutablePath()
        {
        ...
        static string ResolvePathExecutable()
        {
            var executableName = GetExecutableName();
            foreach (var candidate in EnumeratePathCandidates(executableName))
            {
        ...
        {
            var platform = UnityEngine.Application.platform;
            return platform == UnityEngine.RuntimePlatform.WindowsEditor ||
                   platform == UnityEngine.RuntimePlatform.WindowsPlayer
                ? "ffmpeg.exe"
                : "ffmpeg";
        }

        static IEnumerable<string> EnumeratePathCandidates(string executableName)
        {
            ...
            foreach (var rawDirectory in path.Split(Path.PathSeparator))
            {
                ...
                try
                {
                    candidate = Path.Combine(rawDirectory.Trim(), executableName);
                }
                catch
                {
                    continue;
